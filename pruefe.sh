#!/usr/bin/env bash
# pruefe.sh — prüft die aktuelle Auslieferung von https://myowt.io gegen dieses Repository.
#   ./pruefe.sh            aktuelle Auslieferung
#   ./pruefe.sh 1.6.0      bestimmte Version aus releases/
set -uo pipefail
cd "$(dirname "$0")"
HOST=${OWT_HOST:-https://myowt.io}
...
for f in latest.json SHA256SUMS SHA256SUMS.minisig SHA256SUMS.mldsa; do
  curl -fsS --max-time 20 "$HOST/.well-known/owt-release/$f" -o "$tmp/$f" || { alarm "$f nicht abrufbar"; }
done
...
if python3 -c 'import dilithium_py' 2>/dev/null && [[ -s "$tmp/SHA256SUMS.mldsa" ]]; then
  python3 werkzeug/omega-pqsign.py verify "$tmp/SHA256SUMS" "$tmp/SHA256SUMS.mldsa" keys/omega-mldsa.pub >/dev/null 2>&1 && ok "ML-DSA-65-Signatur gültig" || alarm "ML-DSA-65-Signatur UNGÜLTIG"
