...
Signierte Auslieferungen des **OWT Portals** (https://myowt.io) – der öffentliche Prüfpunkt.
...
| `keys/minisign.pub`, `keys/omega-mldsa.pub` | öffentliche Schlüssel – bewusst hier und nicht auf dem Server |
| `werkzeug/omega-pqsign.py` | prüft (und erzeugt) ML-DSA-Signaturen; braucht `python3` und `dilithium-py` |
| `pruefe.sh` | holt Manifest und Signaturen von myowt.io, prüft beide Signaturen, vergleicht mit diesem Repo und hasht die live ausgelieferten Dateien |

## Prüfen

```bash
git clone https://github.com/Omega-Secure/owt-portal-releases.git
cd owt-portal-releases
./pruefe.sh
```
...
Die Prüfseite im Portal: https://myowt.io/pruefen
